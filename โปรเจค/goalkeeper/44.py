...
def register_page():
    ...
    def register_user():
        ...
        # (อัปเดต) ตรวจสอบ Username
        if len(new_user) < 8:
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ต้องมีอย่างน้อย 8 ตัวอักษร")
            return
        if not re.search(r"[A-Z]", new_user):
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ต้องมีตัวพิมพ์ใหญ่ (A-Z) อย่างน้อย 1 ตัว")
            return
        if not re.search(r"[0-9]", new_user):
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ต้องมีตัวเลข (0-9) อย่างน้อย 1 ตัว")
            return

        # (อัปเดต) ตรวจสอบ Email
        if "@" not in email or "." not in email:
            messagebox.showwarning("Email ไม่ถูกต้อง", "รูปแบบอีเมลไม่ถูกต้อง (ต้องมี @ และ .)")
            return

        # (อัปเดต) ตรวจสอบ Password
        special_chars = r"[!@#$%^&*(),.?\":{}|<>]"
        if len(new_pass) < 8:
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ต้องมีอย่างน้อย 8 ตัวอักษร")
            return
        if not re.search(r"[A-Z]", new_pass):
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ต้องมีตัวพิมพ์ใหญ่ (A-Z) อย่างน้อย 1 ตัว")
            return
        if not re.search(special_chars, new_pass):
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ต้องมีอักษรพิเศษ (เช่น !@#$) อย่างน้อย 1 ตัว")
            return
    ...
    tk.Label(form_frame, text="อีเมล (ต้องมี @ และ .)", **label_style).pack(pady=(10, 3))
    ...
    tk.Label(form_frame, text="ชื่อผู้ใช้ (ขั้นต่ำ 8 ตัว, ต้องมี A-Z และ 0-9)", **label_style, wraplength=300).pack(pady=(10, 3))
    entry_new_user = tk.Entry(form_frame, **entry_style)
    entry_new_user.pack(pady=3, ipady=4)

    tk.Label(form_frame, text="รหัสผ่าน (ขั้นต่ำ 8 ตัว, ต้องมี A-Z และ !@#$)", **label_style, wraplength=300).pack(pady=(10, 3))
...
def open_cart_page():
    ...
    tk.Label(price_summary_frame, textvariable=subtotal_var, 
             font=FONT_BODY, 
             bg=BG_COLOR, 
             fg=TEXT_COLOR, anchor="e").pack(fill="x", anchor="e")
    ...
    tk.Label(price_summary_frame, text=f"ภาษีมูลค่าเพิ่ม (VAT 7%):", 
             ...
             bg=BG_COLOR, 
             fg=TEXT_COLOR, anchor="e").pack(fill="x", anchor="e")
    tk.Label(price_summary_frame, textvariable=vat_var, 
             font=FONT_BODY, 
             bg=BG_COLOR, 
             fg=TEXT_COLOR, anchor="e").pack(fill="x", anchor="e")
    ...
    tk.Label(price_summary_frame, textvariable=grand_total_var, 
             font=FONT_HEADING, 
             bg=BG_COLOR, 
             fg=THEME["accent_orange"], anchor="e").pack(fill="x", anchor="e")
    ...
    def rebuild_cart_display():
        ...
        for idx, item in enumerate(cart):
            ...
            tk.Button(right_details_frame, text="<", 
                      ...
                      command=lambda i=idx: decrement_cart_item(i) 
                      ).pack(side="left", ipady=2) 

            tk.Label(right_details_frame, text=f"x {item['quantity']}", 
                     bg=LIST_BG, fg=TEXT_COLOR, 
                     font=FONT_HEADING, width=4).pack(side="left", padx=10) 
            ...
            tk.Button(right_details_frame, text=">", 
                      ...
                      relief="flat", bd=0,
                      activebackground=THEME["bg_list"], 
                      command=lambda i=idx: increment_cart_item(i) 
                      ).pack(side="left", ipady=2) 

            tk.Label(right_details_frame, text=f"รวม: {subtotal:,.0f}฿", 
                     bg=LIST_BG, fg=THEME["accent_orange"], 
                     font=FONT_BUTTON, width=10).pack(side="left", padx=20) 
        ...
        current_grand_total.set(grand_total) 
...
def open_product_page():
    ...
    def populate_products(filter_term=None, filter_category="All"):
        """
        (ใหม่) ล้างและวาดการ์ดสินค้าใหม่ทั้งหมด
        (ถ้ามี filter_term จะกรองก่อนวาด)
        """
    ...
    # (ใหม่) ฟังก์ชันสำหรับเปิดหน้าโปรไฟล์
...
def open_profile_page(parent):
    ...
    profile_pic_label = tk.Label(left_pane, text="[No Image]", 
                                 bg=THEME["bg_list"], fg=THEME["text_primary"], 
                                 relief="ridge")
    profile_pic_label.pack(expand=True, padx=20, pady=20) 
    ...
    try:
        ...
        profile_pic_label.config(font=FONT_HEADING) 
...
def build_product_management(parent):
    ...
    products_tree.heading("category", text="หมวดหมู่") 
    ...
    products_tree.column("category", width=100, anchor="center") 
    ...
    tk.Button(btn_frame, text="แก้ไขหมวดหมู่ 🗂️", command=edit_category, bg=THEME["accent_orange"], **button_config).pack(side="left", padx=5) 
...
#
# 🔽🔽🔽 (อัปเดตล่าสุด) ฟังก์ชันนี้ถูกแก้ไขเป็น Card Layout 🔽🔽🔽
#
...
def build_sales_dashboard(parent):
    ...
    confirmed_orders_tree.pack(side="left", fill="both", expand=True, pady=10)
    
    #
    # 🔽🔽🔽 (อัปเดตล่าสุด) เพิ่ม Event ดับเบิลคลิก 🔽🔽🔽
    #
    def on_order_select(event):
        """(ใหม่) เมื่อดับเบิลคลิกที่ตารางออเดอร์ ให้เปิด view_details_popup"""
        try:
            selected_item = confirmed_orders_tree.focus() 
            if not selected_item:
                return
            order_id = int(selected_item) 
            view_details_popup(parent, order_id) # <--- เรียกใช้ฟังก์ชัน Pop-up
        except Exception as e:
            print(f"Error on order select: {e}")

    confirmed_orders_tree.bind("<Double-1>", on_order_select) # <--- (ใหม่)
    #
    # 🔼🔼🔼 (อัปเดตล่าสุด) สิ้นสุดส่วนที่เพิ่ม 🔼🔼🔼
    #
    ...
    def refresh_sales_data():
        ...
        # --- (ใหม่) Update Confirmed Orders Treeview ---
        confirmed_orders_tree.delete(*confirmed_orders_tree.get_children())
        for order in filtered_orders:
            item_count_in_order = sum(item.get('quantity', 1) for item in order['items'])
