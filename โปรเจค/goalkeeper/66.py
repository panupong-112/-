...
def setup_database():
    ...
    try:
        ...
        print("เพิ่มคอลัมน์ receipt_path ในตาราง orders แล้ว")
...
def load_products():
    ...
    if not products:
        default_products = [
            {"name": "Nike Match", "price": "1,200฿", "img": "images/Nike Match.jpg"},
            {"name": "Nike Match jr", "price": "1,500฿", "img": "images/Nike Match jr.jpg"},
            {"name": "NIKE Vapor Grip", "price": "1,800฿", "img": "images/zzzz.jpg"},
            {"name": "NIKE Grip 3", "price": "2,000฿", "img": "images/zzz.jpg"},
            {"name": "Predator Pro ", "price": "2,500฿", "img": "images/y.jpg"},
            {"name": "Predator Pro ", "price": "2,800฿", "img": "images/R.jpg"},
            {"name": "Predator PRO", "price": "1,200฿", "img": "images/RR.jpg"},
            {"name": "Predator GL PRO", "price": "1,500฿", "img": "images/RRR.jpg"},
            {"name": "PUMA Future Grip 1", "price": "1,800฿", "img": "images/pppppppp.jpg"},
            {"name": "PUMA ULTRA Match RC", "price": "2,000฿", "img": "images/pppppp.jpg"},
            {"name": "PUMA ULTRA Match NC", "price": "2,200฿", "img": "images/ppppp.jpg"},
            {"name": "PUMA FUTURE Ultimate", "price": "2,500฿", "img": "images/pppp.jpg"},
            {"name": "Reusch Pure Contact Fusion", "price": "2,800฿", "img": "images/AAAA.jpg"},
        ]
        for p in default_products:
            cursor.execute("INSERT INTO products (name, price, img) VALUES (?, ?, ?)", 
...
def register_page():
    ...
    #
    # 🔽🔽🔽 (อัปเดตล่าสุด) แก้ไขฟังก์ชันนี้เพื่อเพิ่ม Validation 🔽🔽🔽
    #
    def register_user():
        ...
        # 1. (เดิม) ตรวจสอบว่ากรอกข้อมูลครบหรือไม่
        ...
        #
        # 🔽🔽🔽 (ใหม่) ส่วนตรวจสอบ Validation 🔽🔽🔽
        #
        
        # 2. (ใหม่) ตรวจสอบ Username
        if len(new_user) < 8:
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ")
            return
        if not re.search(r"[A-Z]", new_user):
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ")
            return
        if not re.search(r"[0-9]", new_user):
            messagebox.showwarning("Username ไม่ถูกต้อง", "ชื่อผู้ใช้ (Username) ")
            return

        # 3. (ใหม่) ตรวจสอบ Email
        if "@" not in email or "." not in email:
            messagebox.showwarning("Email ไม่ถูกต้อง", "รูปแบบอีเมลไม่ถูกต้อง (ต้องมี @ และ .)")
            return

        # 4. (ใหม่) ตรวจสอบ Password
        special_chars = r"[!@#$%^&*(),.?\":{}|<>]"
        if len(new_pass) < 8:
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ")
            return
        if not re.search(r"[A-Z]", new_pass):
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ")
            return
        if not re.search(special_chars, new_pass):
            messagebox.showwarning("Password ไม่ถูกต้อง", "รหัสผ่าน (Password) ")
            return

        #
        # 🔼🔼🔼 (ใหม่) สิ้นสุดการตรวจสอบ 🔼🔼🔼
        #

        # 5. (เดิม) ตรวจสอบว่า Username ซ้ำหรือไม่
        ...
        reg_win.destroy()
    #
    # 🔼🔼🔼 (อัปเดตล่าสุด) สิ้นสุดฟังก์ชันที่แก้ไข 🔼🔼🔼
    #
...
def open_cart_page():
    ...
    total_price = 0
    # (อัปเดต) แสดงรายการสินค้าในตะกร้า (พร้อมจำนวน)
    for idx, item in enumerate(cart):
        ...
        tk.Label(left_details_frame, text=item["name"], 
    ...
    # --- 3. ส่วนล่าง: ยอดรวมและปุ่ม (อัปเดต: เพิ่ม VAT) ---
    bottom_frame = tk.Frame(cart_win, bg=BG_COLOR)
    bottom_frame.pack(fill="x", pady=20, padx=40)
    
    subtotal_price = total_price
    vat_amount = subtotal_price * 0.07
    grand_total_price = subtotal_price + vat_amount
    
    price_summary_frame = tk.Frame(bottom_frame, bg=BG_COLOR)
    price_summary_frame.pack(side="left", padx=20)

    tk.Label(price_summary_frame, text=f"ยอดรวม (Subtotal): {subtotal_price:,.2f} ฿", 
             font=FONT_BODY, 
             bg=BG_COLOR, 
             fg=TEXT_COLOR, anchor="e").pack(fill="x", anchor="e")
             
    tk.Label(price_summary_frame, text=f"ภาษีมูลค่าเพิ่ม (VAT 7%): {vat_amount:,.2f} ฿", 
             font=FONT_BODY, 
             bg=BG_COLOR, 
             fg=TEXT_COLOR, anchor="e").pack(fill="x", anchor="e")

    tk.Label(price_summary_frame, text=f"ยอดรวมสุทธิ: {grand_total_price:,.2f} ฿", 
             font=FONT_HEADING, 
             bg=BG_COLOR, 
             fg=THEME["accent_orange"], anchor="e").pack(fill="x", anchor="e")
             
    tk.Button(bottom_frame, text="ยืนยันการชำระเงิน", 
              bg=THEME["accent_green"], fg=TEXT_COLOR,
              font=FONT_HEADING, 
              relief="flat",
              pady=10, padx=20,
              command=lambda: open_qr_code_page(cart_win, grand_total_price)).pack(side="right", padx=20) 
    
    cart_win.protocol("WM_DELETE_WINDOW", cart_win.destroy)

def open_product_page():
    # (อัปเดต) ฟังก์ชันสำหรับปุ่ม + / -
    ...
    # (อัปเดต) ฟังก์ชันเพิ่มลงตะกร้า
    def add_product_to_cart(product_id, products_list):
        global cart, product_quantity_vars
        
        quantity = product_quantity_vars[product_id].get()
        ...
        messagebox.showinfo("เพิ่มลงตะกร้า", f"เพิ่ม {product['name']} (จำนวน {quantity} ชิ้น) ลงตะกร้าแล้ว ✅")
        
        product_quantity_vars[product_id].set(0)

    # (ใหม่) ฟังก์ชันสำหรับเปิดหน้าประวัติการสั่งซื้อ
    ...
    #
    # 🔽🔽🔽 (อัปเดตล่าสุด) เพิ่มปุ่ม "โปรไฟล์" ตรงนี้ 🔽🔽🔽
    #
    
    # (อัปเดต) ย้ายปุ่มตะกร้าไปขวาสุด
    tk.Button(top_bar, text="ดูตะกร้า 🛒", bg=THEME["accent_orange"], fg=THEME["text_primary"], 
              font=FONT_BUTTON,
              relief="flat",
              activebackground="#E67E22",
              command=open_cart_page).pack(side="right", padx=30, pady=30, ipady=5) 
    ...
    for p in products:
        ...
        tk.Button(frame, text="เพิ่มลงตะกร้า", 
                  ...
                  relief="flat",
                  activebackground="#27AE60",
                  command=lambda pid=product_id: add_product_to_cart(pid, products)
                  ).pack(pady=5, ipady=3) 
...
def open_order_history(parent):
    ...
    for order in user_orders:
        ...
        if order['status'] == "Confirmed":
            ...
            tk.Button(right_frame, text="ดูใบเสร็จ", 
                      font=FONT_BUTTON,
                      bg=THEME["accent_blue"], fg=THEME["text_primary"],
                      relief="flat",
...
#
# 🔽🔽🔽 (อัปเดตล่าสุด) เพิ่มฟังก์ชันนี้สำหรับหน้า Sales 🔽🔽🔽
#
def load_confirmed_orders():
    ...
    return orders
#
# 🔼🔼🔼 (อัปเดตล่าสุด) สิ้นสุดฟังก์ชันใหม่ 🔼🔼🔼
#


# (อัปเดต) ฟังก์ชันนี้ต้องรับ receipt_path ได้
...
def generate_order_pdf(order_id):
    ...
    try:
        ...
        for item in order['items']:
            ...
            c.drawString(inch * 1.2, y, item['name'])
...
def build_product_management(parent):
    ...
    products_tree.column("image", width=250)
    ...
    tk.Label(right_pane_frame, text="รูปภาพสินค้า", 
             font=FONT_HEADING, 
             fg=THEME["text_primary"], 
             bg=THEME["bg_secondary"]).pack(pady=10)
    ...
    def refresh_product_list():
        ...
        for p in products:
            ...
            products_tree.insert("", tk.END, iid=p['id'], 
                                 values=(p['id'], p['name'], p['price'], img_name))
    ...
    def edit_price():
        ...
        new_price = simpledialog.askstring("แก้ไขราคา", f"ราคาสินค้าใหม่สำหรับ {current_product['name']} (เช่น 1500฿):", initialvalue=current_product['price'])
    ...
    tk.Button(btn_frame, text="+ เพิ่มสินค้า", command=add_product, bg=THEME["accent_green"], **button_config).pack(side="left", padx=10)
    tk.Button(btn_frame, text="แก้ไขราคา ✏️", command=edit_price, bg=THEME["accent_orange"], **button_config).pack(side="left", padx=10)
    tk.Button(btn_frame, text="- ลบสินค้า 🗑️", command=delete_product, bg=THEME["accent_red"], **button_config).pack(side="left", padx=10)
...
def build_order_management(parent):
    ...
    #
    # 🔽🔽🔽 (อัปเดตล่าสุด) นี่คือฟังก์ชันใหม่สำหรับ Pop-up 🔽🔽🔽
    #
    def view_details_popup(order_id):
        """(ใหม่) สร้างหน้าต่าง Pop-up แสดงรายละเอียดและสลิป"""
        orders = load_orders() 
        order = next((o for o in orders if o['id'] == order_id), None)
    ...
    def populate_orders_frame():
        ...
        for order in orders:
            ...
            btn_details = tk.Button(right_frame, text="ดูรายละเอียด / สลิป 📄", 
                                    ...
                                    bg=THEME["accent_blue"], fg=THEME["text_primary"],
                                    relief="flat",
                                    command=lambda o_id=order_id: view_details_popup(o_id) # <--- (แก้ไข)
                                    )
...
#
# 🔽🔽🔽 (อัปเดตล่าสุด) ฟังก์ชันนี้ถูกแก้ไขค่าเริ่มต้นของ Filter 🔽🔽🔽
#
def build_sales_dashboard(parent):
    """(อัปเดต) สร้างหน้า UI สำหรับสรุปยอดขาย (ดีไซน์ใหม่)"""
    
    # Nonlocal variable เพื่อเก็บรายการออเดอร์ที่ถูกกรองแล้ว
    _current_filtered_orders_data = [] 

    # --- Binding Function (แก้ไขการแปลงข้อมูล) ---
    def show_order_details(event):
        """แสดงรายละเอียดสินค้าในออเดอร์ที่ถูกเลือก"""
        # 1. ล้างตารางรายละเอียดก่อน
        order_detail_tree.delete(*order_detail_tree.get_children())
        selected_order_total_var.set("ยอดรวมออเดอร์: -") 

        selected_item = confirmed_orders_tree.focus()
        if not selected_item:
            return

        # 2. ดึง Order ID
        order_id = confirmed_orders_tree.item(selected_item, 'values')[0] 
        
        # 3. ค้นหาข้อมูล Order ฉบับเต็ม
        selected_order = None
        for order in _current_filtered_orders_data:
        ...
        # 4. อัปเดตยอดรวมออเดอร์ที่เลือก
        selected_order_total_var.set(f"ยอดรวมออเดอร์: {selected_order.get('total', 0.0):,.2f} ฿")
            
        # 5. แสดงผลในตารางรายละเอียดสินค้า
        items_data = selected_order.get('items', [])
        
        # 💡 จุดแก้ไขสำคัญ: ตรวจสอบและพยายามแปลงข้อมูลรายการสินค้าอีกครั้งหากเป็น String
        items_list = []
        if isinstance(items_data, str):
            try:
                items_list = json.loads(items_data) # พยายามแปลง JSON String
                if not isinstance(items_list, list):
                    items_list = [] # ถ้าแปลงได้แต่ไม่ใช่ list ก็ถือว่าผิด
            except json.JSONDecodeError:
                items_list = []
        elif isinstance(items_data, list):
            items_list = items_data # ถ้าเป็น list อยู่แล้วก็ใช้เลย
        
        if not items_list:
            order_detail_tree.insert("", "end", values=("[ไม่พบรายการสินค้าในออเดอร์นี้]", "0", "0.00 ฿", "0.00 ฿"))
            return

        # 6. วนลูปแสดงผลรายการสินค้า
        for item in items_list:
            try:
                # ตรวจสอบว่าเป็น Dictionary เพื่อป้องกันความผิดพลาด
                ...
                # (แก้ไข) ทำความสะอาด price string ก่อนแปลง
                price_str = str(item.get('price', '0.0')).replace('฿', '').replace(',', '')
                unit_price = float(price_str) if price_str.replace('.', '', 1).isdigit() else 0.0 
                
                qty = int(qty) if str(qty).isdigit() else 1
                
                subtotal = unit_price * qty
                
                order_detail_tree.insert("", "end", values=(
                    f"{name} (Size: {item.get('size', 'N/A')})", # <--- (แก้ไข) แสดงไซส์
                    qty,
                    f"{unit_price:,.2f} ฿",
                    f"{subtotal:,.2f} ฿"
                ))
            except Exception as e:
                # หากมีข้อผิดพลาดในการประมวลผลสินค้าแต่ละชิ้น
                order_detail_tree.insert("", "end", values=([f"!!! Data Error: {item.get('name', 'Unknown')}", "0", "0.00 ฿", "0.00 ฿"]))
                pass
    ...
    # --- 1. (ใหม่) ส่วนซ้าย: แผงควบคุมและสรุป ---
    left_pane = tk.Frame(main_pane, bg=THEME["bg_primary"])
    main_pane.add(left_pane, width=350, stretch="never") # ไม่ต้องยืด
    left_pane.pack_propagate(False) # ล็อกความกว้าง

    # --- 1a. Filter Frame ---
    filter_frame = tk.Frame(left_pane, bg=THEME["bg_secondary"], relief="ridge", bd=2)
    filter_frame.pack(fill="x", pady=10, padx=10)
    
    tk.Label(filter_frame, text="ตัวกรองข้อมูล", font=FONT_HEADING, 
             bg=THEME["bg_secondary"], fg=THEME["text_primary"]).pack(pady=10)

    # --- Grid สำหรับตัวกรอง ---
    filter_grid = tk.Frame(filter_frame, bg=THEME["bg_secondary"])
    filter_grid.pack(pady=10, padx=20)
    
    filter_label_style = {"font": FONT_BUTTON, "bg": THEME["bg_secondary"], "fg": THEME["text_primary"]}
    filter_combo_style = {"width": 15, "font": FONT_BODY, "state": "readonly"} 
    filter_entry_style = {"width": 15, "font": FONT_BODY, "relief": "flat", "bd": 0, 
                          "bg": THEME["bg_list"], "fg": THEME["text_primary"], "insertbackground":THEME["text_primary"]}


    # --- ตัวเลือก Year ---
    tk.Label(filter_grid, text="ปี:", **filter_label_style).grid(row=0, column=0, sticky="w", pady=5)
    year_options = ["All"] + [str(y) for y in range(datetime.now().year, 2022, -1)]
    year_var = tk.StringVar(value="All") # <--- (แก้ไข) ค่าเริ่มต้น
    year_combo = ttk.Combobox(filter_grid, textvariable=year_var, values=year_options, **filter_combo_style)
    year_combo.grid(row=0, column=1, sticky="w", padx=5, pady=5)

    # --- ตัวเลือก Month ---
    tk.Label(filter_grid, text="เดือน:", **filter_label_style).grid(row=1, column=0, sticky="w", pady=5)
    month_options = ["All"] + [f"{m:02d} - {datetime(2000, m, 1).strftime('%B')}" for m in range(1, 13)]
    month_var = tk.StringVar(value="All") # <--- (แก้ไข) ค่าเริ่มต้น
    month_combo = ttk.Combobox(filter_grid, textvariable=month_var, values=month_options, **filter_combo_style)
    month_combo.grid(row=1, column=1, sticky="w", padx=5, pady=5)

    # --- ตัวเลือก Day ---
    tk.Label(filter_grid, text="วัน:", **filter_label_style).grid(row=2, column=0, sticky="w", pady=5)
    day_options = ["All"] + [str(d) for d in range(1, 32)]
    day_var = tk.StringVar(value="All") # <--- (แก้ไข) ค่าเริ่มต้น
    day_combo = ttk.Combobox(filter_grid, textvariable=day_var, values=day_options, **filter_combo_style)
    day_combo.grid(row=2, column=1, sticky="w", padx=5, pady=5)

    # --- ตัวเลือก Username ---
    tk.Label(filter_grid, text="ผู้ใช้:", **filter_label_style).grid(row=3, column=0, sticky="w", pady=5)
    user_filter_var = tk.StringVar(value="") 
    user_filter_entry = tk.Entry(filter_grid, textvariable=user_filter_var, **filter_entry_style)
    user_filter_entry.grid(row=3, column=1, sticky="w", padx=5, pady=5)


    # --- 1b. Stats Box Frame (กล่องเดียว) ---
    selected_frame = tk.Frame(left_pane, bg=THEME["bg_secondary"], relief="ridge", bd=2)
    selected_frame.pack(fill="x", pady=20, padx=10)
    
    tk.Label(selected_frame, text="สรุปยอดขายตามที่เลือก", font=FONT_HEADING, 
             bg=THEME["bg_secondary"], fg=THEME["text_primary"]).pack(pady=10)
    ...
    # --- 2. ส่วนขวา: แท็บข้อมูล ---
    right_pane = tk.Frame(main_pane, bg=THEME["bg_primary"])
    main_pane.add(right_pane, stretch="always")

    # (ใหม่) สไตล์สำหรับ Notebook (Tabs)
    style = ttk.Style()
    style.theme_use('default')
    style.configure("TNotebook", background=THEME["bg_primary"], borderwidth=0, highlightthickness=0)
    ...
    style.map("TNotebook.Tab", 
              background=[("selected", THEME["bg_list"])], 
              foreground=[("selected", THEME["text_primary"])])
    
    notebook = ttk.Notebook(right_pane)
    notebook.pack(fill="both", expand=True, padx=10, pady=10)

    # --- 2a. แท็บรายการออเดอร์ (และรายละเอียดสินค้า) ---
    tab1_frame = tk.Frame(notebook, bg=THEME["bg_primary"])
    notebook.add(tab1_frame, text="รายการออเดอร์ที่ยืนยัน")

    # ตารางหลัก: รายการออเดอร์
    cols_orders = ("id", "user", "total", "items", "timestamp")
    confirmed_orders_tree = ttk.Treeview(tab1_frame, columns=cols_orders, show="headings", height=8)
    
    confirmed_orders_tree.heading("id", text="ID")
    confirmed_orders_tree.heading("user", text="ผู้ใช้")
    confirmed_orders_tree.heading("total", text="ยอดรวมสุทธิ")
    confirmed_orders_tree.heading("items", text="จำนวน (ชิ้น)")
    confirmed_orders_tree.heading("timestamp", text="วันที่ยืนยัน")
    
    confirmed_orders_tree.column("id", width=50, anchor="center")
    confirmed_orders_tree.column("user", width=150)
    confirmed_orders_tree.column("total", width=150, anchor="e")
    confirmed_orders_tree.column("items", width=100, anchor="center")
    confirmed_orders_tree.column("timestamp", width=200, anchor="center")
    
    orders_scrollbar = ttk.Scrollbar(tab1_frame, orient="vertical", command=confirmed_orders_tree.yview)
    confirmed_orders_tree.configure(yscrollcommand=orders_scrollbar.set)
    
    orders_scrollbar.pack(side="right", fill="y")
    confirmed_orders_tree.pack(side="top", fill="both", expand=True, pady=10)
    
    # ผูกคำสั่งเมื่อมีการเลือกรายการใน Treeview
    confirmed_orders_tree.bind('<<TreeviewSelect>>', show_order_details) 

    # --- ตารางรายละเอียดสินค้าในออเดอร์ที่เลือก ---
    detail_frame = tk.Frame(tab1_frame, bg=THEME["bg_primary"])
    detail_frame.pack(fill="x", pady=(20, 0), padx=10)
    
    tk.Label(detail_frame, text="รายการสินค้าในออเดอร์", 
             font=FONT_HEADING, bg=THEME["bg_primary"], fg=THEME["text_primary"]).pack(anchor="w", pady=(0, 5))
             
    # Label แสดงยอดรวมสุทธิของออเดอร์ที่ถูกเลือก
    selected_order_total_var = tk.StringVar(value="ยอดรวมออเดอร์: -")
    tk.Label(detail_frame, textvariable=selected_order_total_var, 
             font=FONT_TITLE, bg=THEME["bg_primary"], 
             fg=THEME["accent_green"]).pack(anchor="w", pady=(0, 10))
             
    cols_details = ("name", "quantity", "unit_price", "subtotal")
    order_detail_tree = ttk.Treeview(detail_frame, columns=cols_details, show="headings", height=5)
    
    order_detail_tree.heading("name", text="สินค้า (ไซส์)") # <--- (แก้ไข)
    order_detail_tree.heading("quantity", text="จำนวน")
    order_detail_tree.heading("unit_price", text="ราคาต่อหน่วย")
    order_detail_tree.heading("subtotal", text="ราคารวม")
    
    order_detail_tree.column("name", width=300)
    order_detail_tree.column("quantity", width=80, anchor="center")
    order_detail_tree.column("unit_price", width=120, anchor="e")
    order_detail_tree.column("subtotal", width=120, anchor="e")
    
    detail_scrollbar = ttk.Scrollbar(detail_frame, orient="vertical", command=order_detail_tree.yview)
    order_detail_tree.configure(yscrollcommand=detail_scrollbar.set)
    
    detail_scrollbar.pack(side="right", fill="y")
    order_detail_tree.pack(side="left", fill="both", expand=True)

    # --- 2b. แท็บสินค้ายอดนิยม ---
    tab2_frame = tk.Frame(notebook, bg=THEME["bg_primary"])
    notebook.add(tab2_frame, text="สินค้ายอดนิยม")
    
    cols_bestseller = ("rank", "name", "quantity")
    bestseller_tree = ttk.Treeview(tab2_frame, columns=cols_bestseller, show="headings", height=10)
    ...
    bestseller_scrollbar = ttk.Scrollbar(tab2_frame, orient="vertical", command=bestseller_tree.yview)
    bestseller_tree.configure(yscrollcommand=bestseller_scrollbar.set)
    
    bestseller_scrollbar.pack(side="right", fill="y")
    bestseller_tree.pack(side="left", fill="both", expand=True, pady=10)
    
    
    # --- 3. Data Processing Function ---
    def refresh_sales_data():
        nonlocal _current_filtered_orders_data 
        
        sel_year = year_var.get()
        sel_month = month_var.get()
        sel_day = day_var.get()
        sel_user = user_filter_var.get().strip().lower() 
        ...
        for order in confirmed_orders:
            ...
            # 2. กรองตามเดือน
            month_part = sel_month.split(' ')[0]
            if sel_month != "All" and month_part.isdigit() and order_date.month != int(month_part):
                continue

            # 3. กรองตามวัน
            if sel_day != "All" and sel_day.isdigit() and order_date.day != int(sel_day):
                continue
                
            # 4. กรองตามชื่อผู้ใช้
            if sel_user and sel_user not in order.get('username', '').lower():
                continue
                
            filtered_orders.append(order)
            
            # 5. คำนวณยอดรวม
            total_rev += order.get('total', 0.0) 
            
            # คำนวณยอดรวมสินค้า (รวมถึงการพยายามแปลง JSON สำหรับการคำนวณ)
            ...
            for item in items_list:
                qty = item.get('quantity', 1) # <--- (แก้ไข)
                try:
                    total_itm += int(qty)
                    # (แก้ไข) รวมไซส์เข้ากับชื่อสินค้า
                    item_name = f"{item.get('name', 'N/A')} (Size: {item.get('size', 'N/A')})"
                    product_sales[item_name] = product_sales.get(item_name, 0) + int(qty)
                except (ValueError, TypeError):
                    pass 
                
        # --- เก็บรายการออเดอร์ที่ถูกกรองแล้ว ---
        _current_filtered_orders_data = filtered_orders

        # --- Update Labels (กล่องสรุป) ---
        ...
        # --- Update Confirmed Orders Treeview ---
        confirmed_orders_tree.delete(*confirmed_orders_tree.get_children())
        for order in filtered_orders:
            # คำนวณจำนวนชิ้นทั้งหมดในออเดอร์สำหรับแสดงในตารางหลัก
            items_for_count = order.get('items', [])
            items_list_for_count = items_for_count
            if isinstance(items_for_count, str):
                try:
                    items_list_for_count = json.loads(items_for_count)
                    if not isinstance(items_list_for_count, list): items_list_for_count = []
                except json.JSONDecodeError:
                    items_list_for_count = []

            item_count_in_order = sum(int(item.get('quantity', 1)) for item in items_list_for_count if isinstance(item.get('quantity'), (int, float)) or str(item.get('quantity', '0')).isdigit())
            
            confirmed_orders_tree.insert("", "end", values=(
                order.get('id', 'N/A'),
        ...
        # ล้างตารางรายละเอียดสินค้าเมื่อมีการโหลดข้อมูลใหม่
        order_detail_tree.delete(*order_detail_tree.get_children())
        selected_order_total_var.set("ยอดรวมออเดอร์: -") 
            
    # --- ปุ่ม Refresh/ค้นหา (ผูกคำสั่ง) ---
    tk.Button(filter_frame, text="ค้นหา 🔎", 
              command=refresh_sales_data, 
              bg=THEME["accent_green"], fg=THEME["text_primary"],
              font=FONT_BUTTON, relief="flat",
              width=15, pady=5).pack(pady=10, ipady=5) 
...
def open_admin_dashboard():
    ...
    tk.Label(nav_frame, text="ADMIN", font=FONT_TITLE, fg=THEME["text_primary"], bg=THEME["bg_secondary"]).pack(pady=0, padx=0)
